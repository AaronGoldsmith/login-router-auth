...
const colors = { filled: "#3F51B5", blank: "rgba(255,255,255,0.8)"}

function Star(props) {
  return (
    <div className="starAction" onClick={() => props.handleClick(!props.filled)}>
      <svg width="24" height="24" viewBox="0 0 24 24">
        <path
          fill={props.filled ?  colors.filled : colors.blank}
...
export function UseStar({ prefill = false }) {
  ...
  const _updateFilled = useCallback((updated) => setFilled(updated))
  return <Star filled={filled}
               handleClick={_updateFilled} 
          />
}